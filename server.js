...
const termMatches = (text, terms) => {
  ...
  return terms.reduce((score, term) => score + (normalized.includes(normalize(term)) ? 1 : 0), 0);
};
const matches = (text, terms) => termMatches(text, terms) > 0;
...
function makeActivityHistory(topic, now) {
  ...
  const start = currentBucket - (HISTORY_BUCKETS - 1) * bucketMs;
  const end = currentBucket + bucketMs;
  ...
  for (let timestamp = start; timestamp < end; timestamp += bucketMs) {
    ...
    const activity = bucketDocs.reduce(
      (sum, doc) => sum + termMatches(`${doc.title} ${doc.text}`, topic.terms),
      0
    );
    buckets.push({
  ...
  // Scale against the 90th percentile instead of the single largest bucket.
  // This keeps smaller real changes visible without fabricating activity.
  const values = buckets.map((bucket) => bucket.activity).filter((value) => value > 0).sort((a, b) => a - b);
  const percentileIndex = values.length ? Math.min(values.length - 1, Math.floor(values.length * 0.9)) : 0;
  const scale = values[percentileIndex] || 1;

  return buckets.map((bucket) => ({
    ...bucket,
    intensity: bucket.activity === 0
      ? 0
      : Math.min(100, Math.round((bucket.activity / scale) * 100)),
  }));
}
  ...
  if (url.pathname === "/api/signals") {
    try {
      ...
      json(res, 200, {
        ...
        history: `${HISTORY_BUCKETS} × ${BUCKET_MINUTES}m buckets (~4h window)`,
        ...state,
      });
    } catch (error) {
      json(res, 502, {
